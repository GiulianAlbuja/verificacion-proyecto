...
def step_impl(context):
    ...
    for i in context.producto.puntuaciones_calificaciones:
        calificaciones_totalizados += context.producto.puntuaciones_calificaciones[i]
    ...
    for row in context.table:
        ...
        assert context.producto.puntuaciones_calificaciones[
    ...
    for i in context.producto.puntuaciones_calificaciones:
        ...
        assert porcentajes_calculados == lista_porcentajes_por_estrella[
            i], "No se tiene el porcentaje de calificaciones correcto"
...
def step_impl(context):
    ...
    assert context.producto.puntuaciones_calificaciones[3] == 3, "No se ha calificado correctamente el producto"
...
def step_impl(context):
    context.producto.puntuaciones_calificaciones[3] += 1
    assert context.producto.puntuaciones_calificaciones[3] == 3, "No se ha aumentado la calificación correctamente"
...
def step_impl(context):
    ...
    for row in context.table:
        ...
        assert lista_porcentajes_por_estrella[
                   cantidad_de_estrellas] == porcentaje_de_calificaciones, "No se tiene el porcentaje de calificaciones correcto"
...
def step_impl(context):
    context.cliente = Cliente("1752458974", "Juan", "Herrera", "juan.herrera@hotmail.com", "0984759642",
                              context.pedido)
    context.cliente.calificar_servicio(context.pedido, 3, ["Mal funcionamiento"], context.producto)

    assert context.pedido.servicio.puntuaciones_calificaciones[2]["cantidad"] == 3, "No se ha calificado correctamente el producto"
    return True


@step("seleccione algunas de las siguientes causas de su Calificación del Servicio")
def step_impl(context):
    return True
